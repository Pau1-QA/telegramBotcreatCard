...
def start_command_handler(message: types.Message):
    sti = open ('welcome1.webp', 'rb')
    bot.send_sticker(message.chat.id,sti)
    # отправляем ответ на команду '/start'
    bot.send_message(
        chat_id=message.chat.id, # id чата, в который необходимо направить сообщение
        text='О, привет! 🤟🏻\nА я умею генерировать номер тестовой банковской карты 💳\nВыбери какую карту хочешь:', # текст сообщения
...
def message_handler(message: types.Message):
    ...
    if message.text == 'VISA':
        ...
        # выводим ошибку
        sti = open ('welcome2.webp', 'rb')
        bot.send_sticker(message.chat.id,sti)
        bot.send_message(
            chat_id=message.chat.id,
            text='Не понимаю тебя, попробуй что-нибудь другое',
        )
        return
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=f'Твоя тестовая карта {card_type}:\n<code>{card_number}</code>'
    )
